...
namespace WatchFace.Parser.Utils
{
    public class ParametersConverter
    {
        ...
        public static T Parse<T>(List<Parameter> descriptor, string path = "") where T : new()
        {
            ...
            Logger.Debug("[ParametersConverter.cs] [Parse] Reading parameters in descriptor ({0})", descriptor);
            foreach (var parameter in descriptor)
            {
                Logger.Debug("[ParametersConverter.cs] [Parse] parameter: {0}", parameter);
                var currentPath = string.IsNullOrEmpty(path)
